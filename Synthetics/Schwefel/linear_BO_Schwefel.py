...
for seed in range(replicate):
    ...
    train_X = torch.quasirandom.SobolEngine(dimension=dim,  scramble=True, seed=seed+4).draw(Ninit).to(torch.float64) 
    ...
    torch.save(torch.tensor(regret_y[0:seed+1]), str(dim)+'D_Schwefel_linear_BO_regret1.pt')
