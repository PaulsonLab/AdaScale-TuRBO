...
for seed in range(replicate):
    ...
    while bo<bo_iter:
        if bo == 0:
            X_turbo = torch.quasirandom.SobolEngine(dimension=dim,  scramble=True, seed=seed+7).draw(Ninit).to(device).to(torch.float64)  
        else:
            bo += Ninit
    ...
    torch.save(torch.tensor(regret[0:seed+1]), 'Turbo_lognormal_'+str(dim)+'D_Schwefel_regret1.pt')     
