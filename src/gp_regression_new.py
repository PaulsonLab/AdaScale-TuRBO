...
from gpytorch_modules_new import (
    ...
    get_covar_module_with_dim_scaled_prior_mixed
)
...
class SingleTaskGP_mixed(BatchedMultiOutputGPyTorchModel, ExactGP, FantasizeMixin):
    r"""A single-task exact GP model, supporting both known and inferred noise levels.
    ...
    def __init__(
        ...
        self.mean_module = mean_module
        if covar_module is None:
            covar_module = get_covar_module_with_dim_scaled_prior_mixed(
                ard_num_dims=None,
                batch_shape=self._aug_batch_shape,
                use_rbf_kernel=False,
                dim = transformed_X.shape[-1]
